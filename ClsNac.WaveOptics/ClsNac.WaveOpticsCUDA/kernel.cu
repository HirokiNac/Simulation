...
__global__ void PropFw2D2(const double _k,
	const int _m1, const int _n1, double* _x1, double* _y1, double* _z1, double* _u1re, double* _u1im,
	const int _m2, const int _n2, double* _x2, double* _y2, double* _z2, double* _u2re, double* _u2im)
{
	const int col = blockIdx.x * blockDim.x + threadIdx.x;
	const int row = blockIdx.y*blockDim.y + threadIdx.y;
	//if (_m2 <= col || _n2 <= row)
	//	return;
	const int colrow = col*_n2 + row;
...
__global__ void PropFw1D_f(const double _k,
...
int _n2, float* _x2, float* _y2, float* &_u2re, float* &_u2im)
{
	...
	PropFw1D_f << <numBlock, threadsPerBlock >> >(_k, _n1, dx1, dy1, du1re, du1im, _n2, dx2, dy2, du2re, du2im);
...
//
//	for (int i = 0; i < _n2 / threads; i++)
//	{
//		//2
//		int i0 = i*threads;// *sizeof(double);
//		int p_threads = threads*sizeof(double);
//
//		double *dx2 = 0;
//		cudaMalloc((void**)&dx2, p_threads);
//		cudaMemcpy(dx2, _x2 + i0, p_threads, cudaMemcpyHostToDevice);
//
//		double *dy2 = 0;
//		cudaMalloc((void**)&dy2, p_threads);
//		cudaMemcpy(dy2, _y2 + i0, p_threads, cudaMemcpyHostToDevice);
//
//		double *dz2 = 0;
//		cudaMalloc((void**)&dz2, p_threads);
//		cudaMemcpy(dz2, _z2 + i0, p_threads, cudaMemcpyHostToDevice);
//
//		double *du2re = 0;
//		cudaMalloc((void**)&du2re, p_threads);
//		//cudaMemcpy(du2re, _u2re, memsize2, cudaMemcpyHostToDevice);
//
//		double *du2im = 0;
//		cudaMalloc((void**)&du2im, p_threads);
//		//cudaMemcpy(du2im, _u2im, memsize2, cudaMemcpyHostToDevice);
//
//		PropFw2D << < threadsPerBlock,numBlock >> >(_k, _n1, dx1, dy1, dz1, du1re, du1im, threads, dx2, dy2, dz2, du2re, du2im);
//
//		cudaMemcpy(u2re_out + i0, du2re, p_threads, cudaMemcpyDeviceToHost);
//		cudaMemcpy(u2im_out + i0, du2im, p_threads, cudaMemcpyDeviceToHost);
//
//		cudaFree(dx2);
//		cudaFree(dy2);
//		cudaFree(dz2);
...
int _n2, double* _x2, double* _y2, double* _z2, double* &_u2re, double* &_u2im)
{
	...
	PropFw2D << <_n2 / 1024, 1024 >> >(_k, _n1, dx1, dy1, dz1, du1re, du1im, _n2, dx2, dy2, dz2, du2re, du2im);

	cudaDeviceSynchronize();


	//double* u2re_out = 0;//(double*)malloc(memsize2);
	//cudaMalloc((void**)&u2re_out, memsize2);
	cudaMemcpy(_u2re, du2re, memsize2, cudaMemcpyDeviceToHost);
	//double* u2im_out = 0;//  (double*)malloc(memsize2);
	//cudaMalloc((void**)&u2im_out, memsize2);
	cudaMemcpy(_u2im, du2im, memsize2, cudaMemcpyDeviceToHost);
...
int _m2,int _n2, double* _x2, double* _y2, double* _z2, double* &_u2re, double* &_u2im)
{
	...
	dim3 threadsPerBlocks(1024, 1024);
	dim3 Blocks(numBlock, numBlock);

	PropFw2D2 << <Blocks, threadsPerBlocks >> >(_k, _m1, _n1, dx1, dy1, dz1, du1re, du1im, _m2, _n2, dx2, dy2, dz2, du2re, du2im);
